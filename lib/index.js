...
function calcUnitToViewUnit(weight) {
    return UnitConvertor.convert(getCalcUnitName(), getViewUnitName(), weight);
}

function calcUnitToGymPlatesUnit(weight) {
    return UnitConvertor.convert(getCalcUnitName(), getGymPlatesUnitName(), weight);
}

function calcUnitToGymBarbellUnit(weight) {
    return UnitConvertor.convert(getCalcUnitName(), getGymBarbellUnitName(), weight);
}
...
function newPlatesLoadOption(factoredBarbellWeight, platesCombo) {
    ...
    const totalWeight = calcUnitToViewUnit(factoredBarbellWeight + platesCombo.weight);
    const barbellWeight = calcUnitToGymBarbellUnit(factoredBarbellWeight);
    const deltaInViewUnit = calcUnitToViewUnit(delta);
    const plates = aggregatePlatesLoadCombo(platesCombo.plates).map(plate => {
        plate.weight = calcUnitToGymPlatesUnit(plate.weight);
        return plate;
    });
...
function calcGymPlatesLoadOptionsByTargetWeight(targetWeight) {
    ...
    targetWeight = convertToCalcUnit(getViewUnit().NAME, targetWeight);
    ...
    const barbellWeight = convertToCalcUnit(getGymBarbellUnit().NAME, barbell.weight);
