...
function convertFromGymBarbellUnitToGymPlatesUnit(weight) {
    const gymBarbellUnit = getGymBarbellUnit();
    const gymPlatesUnit = getGymPlatesUnit();
    if (gymBarbellUnit.NAME == gymPlatesUnit.NAME) return weight;
    if (gymBarbellUnit.NAME == state.BASES.KG.NAME) {
        return kgToLb(weight);
    }
    return lbToKg(weight);
}

function convertFromViewUnitToGymPlatesUnit(weight) {
    const viewUnit = getViewUnit();
    const gymPlatesUnit = getGymPlatesUnit();
    if (viewUnit.NAME == gymPlatesUnit.NAME) return weight;
...
function convertFromGymPlatesUnitToViewUnit(weight) {
    const viewUnit = getViewUnit();
    const gymPlatesUnit = getGymPlatesUnit();
    if (viewUnit.NAME == gymPlatesUnit.NAME) return weight;
    if (viewUnit.NAME == state.BASES.KG.NAME) {
...
function newPlatesLoadOption(factoredBarbellWeight, platesCombo) {
    const totalWeight = convertFromGymPlatesUnitToViewUnit(factoredBarbellWeight + platesCombo.weight);
    const barbellWeight = getSelectedBarbell().weight;
    const delta = platesCombo.delta;
    const deltaInViewUnit = convertFromGymPlatesUnitToViewUnit(delta);
...
function calcGymPlatesLoadOptionsByTargetWeight(targetWeight) {
    ...
    targetWeight = convertFromViewUnitToGymPlatesUnit(targetWeight);
    ...
    const barbellWeight = convertFromGymBarbellUnitToGymPlatesUnit(barbell.weight);
    ...
    const gymPlatesOptions = sortByWeightDESC(copySupportedPlates(getGymPlatesUnit()));
...
function updateGymPlatesUnitBadgeText() {
    ...
    badge.innerHTML = getGymPlatesUnit().NAME;
}
...
function createPlatesLoadOptionCardHTML(option, idx, select = false, title) {
    ...
    return `<div class="card mx-auto my-2 mb-3 shadow px-0 col-12" style="max-width: 18rem;">
        ...
        <div class="card-footer bg-dark-c">
            Barbell: ${option.barbellWeight} ${getGymBarbellUnit().NAME}
        </div>
    ...
    function createPlatesLoadHTML() {
        ...
        let platesData =
            ...
            Plates On Each Side - ${getGymPlatesUnit().NAME}
        </div>`;
