...
function convertWeightUnit(id) {
    ...
    if (id == 'lb-to-kg') {
        inputResult.value = String(toFixedNumber(lbToKg(Number(input.value)), 2));
    } else {
        inputResult.value = String(toFixedNumber(kgToLb(Number(input.value)), 2));
    }
}

function convertFromViewUnitToGymUnit(weight) {
    const viewUnit = getViewUnit();
    const gymUnit = getGymUnit();
    if (viewUnit.NAME == gymUnit.NAME) return weight;
    if (viewUnit.NAME == state.BASES.KG.NAME) {
        return kgToLb(weight);
    }
    return lbToKg(weight);
}

function convertFromGymUnitToViewUnit(weight) {
    const viewUnit = getViewUnit();
    const gymUnit = getGymUnit();
    if (viewUnit.NAME == gymUnit.NAME) return weight;
    if (viewUnit.NAME == state.BASES.KG.NAME) {
...
function getViewUnitBarbellWeight(gymUnitBarbellWeight) {
    const gymBarbellOptions = getGymUnit().BARBELLS_OPTIONS;
    for (let i = 0; i < gymBarbellOptions.length; i++) {
        if (gymUnitBarbellWeight === gymBarbellOptions[i].weight) {
            return getViewUnit().BARBELLS_OPTIONS[i].weight;
        }
    }
    return convertFromGymUnitToViewUnit(gymUnitBarbellWeight);
}

function newPlatesLoadOption(barbell, platesCombo) {
    const totalWeight = convertFromGymUnitToViewUnit(barbell.weight + platesCombo.weight);
    const barbellWeight = getViewUnitBarbellWeight(barbell.weight);
    const delta = platesCombo.delta;
    const deltaInViewUnit = convertFromGymUnitToViewUnit(delta);

    return {
        weight: toFixedNumber(totalWeight, 2),
        barbellWeight: toFixedNumber(barbellWeight, 2),
        plates: aggregatePlatesLoadCombo(platesCombo.plates),
...
function calcGymPlatesLoadOptionsByTargetWeight(targetWeight) {
    ...
    targetWeight = convertFromViewUnitToGymUnit(targetWeight);
    ...
    const gymPlatesOptions = sortByWeightDESC(copySupportedPlates(getGymUnit()));
        ...
        for (const plate of gymPlatesOptions) {
            ...
            for (; plate.max >= 2; plate.max -= 2) {
                for (let deltaSign = -1; deltaSign <= 1; deltaSign += 2) {
                    ...
                    platesLoadOptions.push(newPlatesLoadOption(barbell, platesCombo));
                }
            }
...
function updateGymPlatesUnitBadgeText() {
    ...
    badge.innerHTML = getGymUnit().NAME;
}
...
function updateTargetWeightInputPlaceholder() {
    ...
    preText.innerHTML = getViewUnit().NAME;
}
