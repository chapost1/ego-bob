...
const BASE_BY_NAME = {
    ...
    [BASES.POUND.NAME]: state.BASES.POUND
}
...
function convertWeightUnit(id) {
    ...
    if (id == 'pound-to-kg') {
        inputResult.value = String(toFixedNumber(poundToKg(Number(input.value)), 2));
    } else {
        inputResult.value = String(toFixedNumber(kgToPound(Number(input.value)), 2));
    }
}
...
function newPlatesLoadOption(barbell, platesCombo) {
    ...
    const barbellWeight = convertFromGymUnitToViewUnit(barbell.weight);
