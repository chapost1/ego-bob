function UnitConvertor() {
    ...
    // private
    const currToStandardUnitRatio = currUnit => 1 / STANDARD_TO_OTHER_UNITS_RATIOS[currUnit];

    const standardToTargetUnitRatio = targetUnit => STANDARD_TO_OTHER_UNITS_RATIOS[targetUnit];
    
    const maskUnit = unitName => unitName.toUpperCase();
    
    const getConversionRatio = (currUnit, targetUnit) => {
        if (currUnit == targetUnit) return 1;
        return currToStandardUnitRatio(currUnit) * standardToTargetUnitRatio(targetUnit);
    }

    // public
    this.convert = (currUnit, targetUnit, weight) => {
        return weight * getConversionRatio(maskUnit(currUnit), maskUnit(targetUnit));
    }
