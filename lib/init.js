...
$(document).ready(() => requestAnimationFrame(init));
$(document).on("click", () => requestAnimationFrame(init));
// init html and functionality
function init() {
    if (getIsInitializedState()) return;
                    ...
                    for (const plate of base.PLATES_OPTIONS) {
                        ...
                        baseHTML +=
                            `<tr data-weight="${plate.weight}">
                                <td>
                                    <form>
                                        <input disabled type="number" class="form-control w-75" placeholder="0" value="${plate.weight}">
                                    </form>
                                </td>
                                <td>
                                    <form>
                                        <input id="${countID}" onkeyup="${onChangeMethodCall}"
                                         type="number" class="form-control w-75" placeholder="0" value="${plate.max}">
                                    </form>
                                </td>
                            <td scope="col">
                                <div class="form-check form-switch">
                                    <input onchange="${onChangeMethodCall}"
                                     class="form-check-input noselect" type="checkbox" id="${switchID}" ${checked}>
                                </div>
                            </td>
                        </tr>`;
                        index++;
...
function getIsInitializedState() {
    return $("#is-initialized").val();
}

function setIsInitializedState(val) {
    $("#is-initialized").val(val);
}
