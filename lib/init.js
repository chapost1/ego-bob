...
function init() {
                            ...
                            <td scope="col">
                                <div class="form-check form-switch form-switch-md">
                                    <input onchange="${onChangeMethodCall}"
                                     class="form-check-input noselect clickable" type="checkbox" id="${switchID}" ${checked}>
                                </div>
                            </td>
                        </tr>`;
                        index++;
    ...
    (function addModalMarginForIosSafariToPreventAntiBottomClick() {
        const isSafari = !!navigator.userAgent.match(/Version\/[\d\.]+.*Safari/);
        const iOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
        if (isSafari && iOS) {
            const modalContent = $("#workout-wizard-plates-settings-modal").find(".modal-content");
            if (!modalContent[0]) return;
            $(modalContent[0]).addClass("my-5");
            // $(modalContent[0]).addClass("mb-5");
        }
    })();
